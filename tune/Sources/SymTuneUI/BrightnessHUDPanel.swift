...
/// The stand-in for the system brightness HUD (issue #250).
///
/// Shown only when this app answers the keys, so it never appears beside the
/// system's own. The shape is the part that has to match: a non-activating
/// borderless panel above full-screen apps, click-through, on every Space, with
/// a glass backdrop and a 16-segment bar — the same quantisation the system HUD
/// uses.
@MainActor
final class BrightnessHUDPanel {
    ...
    init() {
        ...
        // Purely a readout: every click belongs to whatever is underneath it.
        ...
        if #available(macOS 26.0, *) {
            ...
            // Older macOS still has to render something; vibrancy is closest.
        ...
        NSLayoutConstraint.activate([
            ...
            iconView.centerYAnchor.constraint(equalTo: content.centerYAnchor, constant: 12),
            iconView.widthAnchor.constraint(equalToConstant: 84),
            iconView.heightAnchor.constraint(equalToConstant: 84),
            bar.leadingAnchor.constraint(equalTo: content.leadingAnchor, constant: 22),
            bar.trailingAnchor.constraint(equalTo: content.trailingAnchor, constant: -22),
            bar.bottomAnchor.constraint(equalTo: content.bottomAnchor, constant: -26),
        ])
    }

    /// Show the HUD at `level` (0...1) and schedule the fade-out.
    func show(level: Float) {
        let filled = BrightnessKeyStep.filledSegments(for: level, of: Self.segmentCount)
        for (index, segment) in segments.enumerated() {
            segment.layer?.backgroundColor = index < filled
    ...
    /// Take the HUD off screen at once — the keys just went back to macOS, and
    /// a HUD still fading out there would be this app's readout for the
    /// system's key press.
